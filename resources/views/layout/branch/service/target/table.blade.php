            ...
            @foreach($data as $key => $row)
                <tr>
                    ...
                    @if(getUserLevel($config['privilege']) >= CAN_CRUD)
                        ...
                        @else
                            <td class="w-1-slot">
                                <a href="{{ url($config['url'].'/create/'.$row->id) }}" class="btn btn-primary btn-icon me-2" data-bs-toggle="tooltip" data-bs-title="Generate Target">@svg('heroicon-s-cursor-arrow-ripple', 'icon-sm')</a>
                            </td>
                        @endif
                    @endif
                </tr>
