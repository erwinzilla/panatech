            ...
            <div id="main" class="card">
                <div class="card-header d-flex justify-content-start w-100">
                    ...
                    <div>
                        ...
                        <small class="text-muted">Daftar list cabang</small>
                    </div>
                    {{--                Jika can CRUD maka munculkan tombol--}}
                    @if(getUserLevel('branches') >= CAN_CRUD)
                        <div class="align-self-center ms-auto">
                            @if($type == 'data')
                                <a href="{{ url('branch/create') }}" class="btn btn-primary">
                                    @svg('heroicon-s-plus', 'icon') Create new
                                </a>
                            @endif
                            @if($type == 'trash')
                                <a href="{{ url('branch/restore') }}" class="btn btn-info me-2 restore-action-link">
                                    @svg('heroicon-s-arrow-up-on-square-stack', 'icon') Restore All
                                </a>
                                <a href="{{ url('branch/delete') }}" class="btn btn-danger delete-action-link">
